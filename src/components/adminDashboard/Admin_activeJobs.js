...
function Admin_activeJobs() {
  ...
  useEffect(() => {
    console.log(config)
    axios.get(`http://localhost:9000/api/admin/getActiveJobs`).then((response) => {
      // setData({response})
  ...
  return (
    <div className="pd_a_j_main">
         <div id="tohide"  style={{display: "none"}}>
          <div>No Active Jobs please Approve Some</div>
        </div>
        {/* mapping */}
...
export default Admin_activeJobs;
